...
<script type="text/javaScript">
	$(document).ready(function () {
		$.ajax({
			url: "/lecture/faqCategoryData",
			type: 'GET',
			success: function(data){
				console.log(data);
				for( var i = 0; i < data.length; i++){
					var item = data[i];
					$("#faqCategory").append("<option value=" + item.TYPE_NO +">"+ item.FAQ_TYPE+"</option>");
				}
			},
			error: function(err){
 ...
 -->	
    ...
    <!--  폼검증,중복확인 - jQuery -->
	<script src="${pageContext.request.contextPath }/resources/js/formCheck.js?version=14"></script>	
	...
	switch(result) {
		...
		case '글쓰기-성공' : alert("글쓰기 성공~!!");
				break;
			
		case '글삭제-실패' : alert("권한이 없습니다!");
				break;
				
		case '글삭제-성공' : alert("삭제되었습니다!");
				break;
		
		case '글삭제-비회원성공' : alert("삭제되었습니다!");
				break;
				
		case '글삭제-비회원실패' : alert("삭제실패!");
				break;
